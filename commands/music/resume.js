module.exports = {
    ...
    execute(client, message) {
        ...
        if (!client.player.getQueue(message).paused) return message.channel.send(`${client.emotes.error} - Ya la reanudaste`);

        const success = client.player.resume(message);

        if (success) message.channel.send(`${client.emotes.success} - Cancion ${client.player.getQueue(message).playing.title} reanudada`);
    },
};